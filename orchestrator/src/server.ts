...
function setupRoutes(pgPool: Pool | null) {
    ...
    app.get('/api/v1/health/cross-node', async (_req, res) => {
        try {
            const report = await consensusManager.verifyAcrossNodes();
            res.status(report.consistent ? 200 : 409).json({
                ...report,
                status: report.consistent ? 'NODES_CONSISTENT' : 'NODE_DIVERGENCE_DETECTED',
            });
        } catch (err: any) {
            res.status(503).json({ error: err.message });
        }
